...
resource "aws_route53_record" "explorer_private_reverse" {
  count   = var.explorer_count
  zone_id = aws_route53_zone.reverse_zone.zone_id
  records = [format("explorer-%03d.%s", count.index + 1, var.base_dn)]
  type    = "PTR"
  ttl     = "60"
...
resource "aws_acm_certificate" "smart_contract_verifier" {
  count             = var.route53_zone_id == "" ? 0 : 1
  domain_name       = "smart-contract-verifier.${data.aws_route53_zone.ext_rpc[0].name}"
  validation_method = "DNS"
...
resource "aws_route53_record" "smart_contract_verifier_validation" {
  for_each = {
    for dvo in(var.route53_zone_id == "" ? [] : aws_acm_certificate.smart_contract_verifier[0].domain_validation_options) : dvo.domain_name => {
      name   = dvo.resource_record_name
      record = dvo.resource_record_value
      type   = dvo.resource_record_type
    }
...
resource "aws_acm_certificate_validation" "smart_contract_verifier" {
  count                   = var.route53_zone_id == "" ? 0 : 1
  certificate_arn         = aws_acm_certificate.smart_contract_verifier[0].arn
  validation_record_fqdns = [for record in aws_route53_record.smart_contract_verifier_validation : record.fqdn]
}
...
resource "aws_route53_record" "public_explorer" {
  ...
  type    = "CNAME"
  ttl     = "60"
  records = [var.aws_lb_explorer_domain]
}
...
resource "aws_route53_record" "public_smart_contract_verifier" {
  count   = var.explorer_count
  zone_id = data.aws_route53_zone.ext_rpc[0].zone_id
  name    = "smart-contract-verifier"
  type    = "CNAME"
  ttl     = "60"
  records = [var.aws_lb_smart_contract_verifier_domain]
}
